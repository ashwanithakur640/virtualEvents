...
//   screenClient.init(confernce_params['appID'], function () {
//     screenClient.join(null, confernce_params['channel'], null, function (uid) {
//       // Save the uid of the local stream.
...
//   console.log(' streamSpec '+ JSON.stringify(streamSpec));
...
//     screenStream.play('local_stream');
...
function assignPresenter(uid) {
  ...
  if (uid == organizer_id) {
    //alert( 'is_screen_cvxshared '+ is_screen_shared );
    if (is_screen_shared) {
  ...
  RTMClient.activeChennel.on('ChannelMessage', ({
    ...
    if (message.type == 'organizerLeft') {
      ...
      setTimeout(function () {
        ...
        if($('#prefix').val() == '' ){
      window.location = base_url+"session-hall" ;
    }else if( $('#prefix').val() == 'subadmin'  ){
        window.location = base_url+"admin/dashbaord";
    }else{
        window.location = base_url +$('#prefix').val() + "/dashboard";
    }
