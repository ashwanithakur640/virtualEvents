...
function addView (jonee, show) {
  ...
  if (!$("#remote_video_panel_" + jonee.id)[0]) {
    ...
    if(remote_controls_access==1) {
      ...
      div = `<li id="remote_video_panel_`+jonee.id+`">
      <div class="col-sm-12">
...
function handleEvents (rtc) {
  ...
  rtc.client.on("stream-subscribed", function (evt) {
    ...
    $.ajax({url:  base_url+"getUser/"+id, success: function(result){
  ...
  getDevices(async function (devices) {
    ...
    if (validator(confernce_params, fields)) {
      ...
      if(time_left==0){
        ...
        //if(!alert('Conference will be started at '+time_left)) document.location = '/my-conferences';
...
screenClient.init(confernce_params['appID'], function() {
    screenClient.join(null, confernce_params['channel'], "0"+confernce_params['uid'], function(uid) {
        // Save the uid of the local stream.
        localStreams.push(uid);
        ss_uid = uid;
        //console.log('==ss_uid', ss_uid);
       $('#uid_sharescreen').val(ss_uid);
...
screenStream.init(function() {
   ...
   //$('#uid_sharescreen').val(screenStream.streamId);
