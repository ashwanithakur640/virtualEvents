...
`).appendTo('#participants');
    ...
    function startrecording(refresh = false) {
      ...
      $.ajax({
        url: base_url + "cloud-recording-acquire?cname=" + cname + "&conference_id=" + conference_id + "&screen_id=" + screen_share_id,
        success: function (result) {
    ...
    function stoprecording() {
      ...
      $("#stop-recording").hide();
      $.ajax({
        url: base_url + "cloud-recording-stop?video_streaming_id=" + video_streaming_id,
        success: function (result) {
          //console.log(result+'======startrecording1');
          //var result_arr = JSON.parse(result);
    ...
    function stoprecordingRefresh() {
      $.ajax({
        url: base_url + "cloud-recording-stop-refresh/" + conference_id,
        success: function (result) {},
        error: function (err) {}
      });
    }
    ...
    $(function () {
      ...
      confernce_params['appID'] = appId; confernce_params['cameraId'] = $("#cameraId").val(); confernce_params['cameraResolution'] = "4K_3"; confernce_params['channel'] = $("#channel").val(); confernce_params['codec'] = "h264"; confernce_params['microphoneId'] = $("#microphoneId").val(); confernce_params['mode'] = "live"; confernce_params['uid'] = user.id;
...
screenClient.init(confernce_params['appID'], function() {
    screenClient.join(null, confernce_params['channel'],"1"+confernce_params['uid'], function(uid) {
        ...
        ss_uid = confernce_params['uid'];
       alert(ss_uid+"");
     
       console.log(screenClient.publish(screenStream),"=== screen stream");
...
screenStream.init(function() {
   ...
   //$('#share-screen-id').val(screenStream.getId());
    console.log(screenClient.publish(screenStream),"=== share screen published");
    //console.log("=========error1")
    var msg = JSON.stringify(message);
    RTMClient.activeChennel.sendMessage({ text: msg}).then(() => {
      //console.log(msg+'======startrecording1');
   ...
   // $('#slider').data('flexslider').addSlide('<li> '+$("#local_stream").html()+' < /li>');
    ...
    if (is_screen_shared) {
      ...
      $("#local_stream_container").prepend(' <div id = "remote-streams" > < /div>');
        ...
        $('#share_screen_user_' + uid).show(); $('.video-upper-btns').addClass('remote-presenting'); $('.current_user_presenter_' + uid).show(); $("#presenter-list option[value=" + uid + "]").attr('selected', 'selected'); $('#share-screen-out').prepend($("#remote_video_panel_" + user_id)); $("#remote_video_panel_" + user_id).find('.testimonial-lower-outr').addClass('no-clickable');
      }
      else {
        $('.shared_screen_presenter').hide();
        $("#remote-streams").remove();
        $("#local_stream_container").prepend('< div id = "remote-streams" > < /div>');
          $("#remote-streams").prepend($("#remote_video_panel_" + uid)); $('#share_screen_user_' + uid).show(); $('.video-upper-btns').addClass('remote-presenting'); $('.current_user_presenter_' + uid).show(); $("#presenter-list option[value=" + uid + "]").attr('selected', 'selected'); $("#make_presenter_" + uid).addClass('active');
        }
      }
    }
