...
class ReportController extends Controller
{
            ...
            return DataTables::of($data)
                        ...
                        ->addColumn('action', function ($modal) {    
                            ...
                            if(isset($modal->answer)){
                               $additional = '<a href="'.url($this->Prefix."/faq-report/view/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>&nbsp';
                            } else{
                            	$additional = '<a href="'.url($this->Prefix."/reply-answer/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="Reply"><i class="fa fa-share"></i></a>&nbsp';
                            }            
                            $action = $additional.'<a class="deleteRecord" data-toggle="tooltip" title="Delete" style="color: #4e73df;"><i class="fa fa-trash"></i></a>
                                <form action="'.url($this->Prefix.'/faq-report-delete/'.Helper::encrypt($modal->id)).'" method="POST" style="display: none;">
    ...
    public function replyAnswer($encryptId)
    {
        ...
        try{
            return view('vendors.reports.replyAnswer', compact('data'));
        } catch(\Exception $e) {
     ...
     */
    public function updateAnswer($prefix , Request $request, $encryptId)
    {
        $id = Helper::decrypt($encryptId);
        ...
        try{
            ...
            return redirect($prefix.'/faq-report/')->withSuccess('FAQ answer has been sent successfully');
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function show($prefix, $encryptId)
    {
        ...
        $data = Faq::findOrFail($id);
        try{
            return view('vendors.reports.view', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function indexContactUs(Request $request){
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                        ...
                        ->addColumn('action', function ($modal) {    
                            $additional = '';
                            if(isset($modal->answer)){
                               $additional = '<a href="'.url($this->Prefix."contact-us-report/view/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>&nbsp';
                            } else{
                                $additional = '<a href="'.url($this->Prefix."/contact-us-report/reply-answer/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="Reply"><i class="fa fa-share"></i></a>&nbsp';
                            }            
                            $action = $additional.'<a class="deleteRecord" data-toggle="tooltip" title="Delete" style="color: #4e73df;"><i class="fa fa-trash"></i></a>
                                <form action="'.url($this->Prefix.'/contact-us-report/delete/'.Helper::encrypt($modal->id)).'" method="POST" style="display: none;">
                                    <input type="hidden" name="_token" value="'. csrf_token() .'">
                                    <input type="hidden" name="_method" value="DELETE">
                                </form>';
                             return $action;
        ...
        return view('vendors.reports.contactUs.index');
    }
     ...
     */
    public function replyContactUsAnswer($Prefix , $encryptId)
    {
        $id = Helper::decrypt($encryptId);
        $data = ContactUs::findOrFail($id);
        try{
            return view('vendors.reports.contactUs.replyAnswer', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
     ...
     */
    public function updateContactUsAnswer($prefix , Request $request, $encryptId)
    {
        $id = Helper::decrypt($encryptId);
        ...
        try{
            ...
            return redirect($this->Prefix.'/contact-us-report/')->withSuccess('Contact Us answer has been sent successfully');
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
     ...
     */
    public function showContactUs($Prefix , $encryptId)
    {
        $id = Helper::decrypt($encryptId);
        $data = ContactUs::findOrFail($id);
        try{
            return view('vendors.reports.contactUs.view', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
     ...
     */
    public function destroyContactUs($Prefix ,  $encryptId)
    {
        $id = Helper::decrypt($encryptId);
        $data = ContactUs::findOrFail($id);
        try{
