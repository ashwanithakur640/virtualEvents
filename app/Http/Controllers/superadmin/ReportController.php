...
class ReportController extends Controller
{
            ...
            return DataTables::of($data)
                        ...
                        ->addColumn('action', function ($modal) {    
                            ...
                            if(isset($modal->answer)){
                               $additional = '<a href="'.url("superadmin/faq-report/view/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>&nbsp';
                            } else{
                            	$additional = '<a href="'.url("superadmin/faq-report/reply-answer/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="Reply"><i class="fa fa-share"></i></a>&nbsp';
                            }            
                            $action = $additional.'<a class="deleteRecord" data-toggle="tooltip" title="Delete" style="color: #4e73df;"><i class="fa fa-trash"></i></a>
                                <form action="'.url('superadmin/faq-report/delete/'.Helper::encrypt($modal->id)).'" method="POST" style="display: none;">
    ...
    public function replyAnswer($encryptId)
    {
        ...
        try{
            return view('superadmin.reports.replyAnswer', compact('data'));
        } catch(\Exception $e) {
    ...
    public function updateAnswer(Request $request, $encryptId)
    {
        ...
        try{
            ...
            return redirect('superadmin/faq-report/')->withSuccess('FAQ answer has been sent successfully');
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function show($encryptId)
    {
        ...
        $data = Faq::findOrFail($id);
        try{
            return view('superadmin.reports.view', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function indexContactUs(Request $request){
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                        ...
                        ->addColumn('action', function ($modal) {    
                            $additional = '';
                            if(isset($modal->answer)){
                               $additional = '<a href="'.url("superadmin/contact-us-report/view/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>&nbsp';
                            } else{
                                $additional = '<a href="'.url("superadmin/contact-us-report/reply-answer/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="Reply"><i class="fa fa-share"></i></a>&nbsp';
                            }            
                            $action = $additional.'<a class="deleteRecord" data-toggle="tooltip" title="Delete" style="color: #4e73df;"><i class="fa fa-trash"></i></a>
                                <form action="'.url('superadmin/contact-us-report/delete/'.Helper::encrypt($modal->id)).'" method="POST" style="display: none;">
                                    <input type="hidden" name="_token" value="'. csrf_token() .'">
                                    <input type="hidden" name="_method" value="DELETE">
                                </form>';
                             return $action;
        ...
        return view('superadmin.reports.contactUs.index');
    }
    ...
    public function replyContactUsAnswer($encryptId)
    {
        ...
        try{
            return view('superadmin.reports.contactUs.replyAnswer', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function updateContactUsAnswer(Request $request, $encryptId)
    {
        ...
        try{
            ...
            return redirect('superadmin/contact-us-report/')->withSuccess('Contact Us answer has been sent successfully');
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function showContactUs($encryptId)
    {
        ...
        $data = ContactUs::findOrFail($id);
        try{
            return view('superadmin.reports.contactUs.view', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
