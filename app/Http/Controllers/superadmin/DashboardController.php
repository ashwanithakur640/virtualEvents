...
class DashboardController extends Controller
{
    ...
    public function index(){
        $vendors = User::where('role_id', '1')->count();
        $customers = User::where('role_id', '2')->count();
        $events = Event::count();
        $today = date('Y-m-d');
        $todayEvents = Event::where('start_date_time','<=', $today)->where('end_date_time','>=', $today)->count();
        $todayEventsDetails = Event::where('start_date_time','<=', $today)->where('end_date_time','>=', $today)->get();
        $holdEvents = Event::where('status','=', 'Onhold')->count();
        $rescheduledEvents = Event::where('rescheduled','=', Event::RESCHEDULED)->count();
        $currentWeekEvents = Event::count();
        $futureEvents = Event::where('start_date_time','>', $today)->count();
        /* current weeks date */
        ...
        $weekEvents = Event::where('start_date_time','>=', $weekStartDate)->where('end_date_time','<=', $weekEndDate)->count();
        $pastEvents = Event::where('end_date_time','<', $today)->count();
    	return view('superadmin.dashboard', compact('vendors', 'customers', 'events' , 'todayEvents' , 'futureEvents' , 'weekEvents' , 'todayEventsDetails', 'pastEvents' , 'holdEvents' , 'rescheduledEvents'));
    }
    ...
    public function editProfile(){
        ...
        return view('superadmin.editProfile', compact('data'));
    }
    ...
    public function updatePassword(Request $request){
        ...
        try{
            ...
            return redirect('superadmin/login')->withSuccess('Password has been changed successfully');
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
