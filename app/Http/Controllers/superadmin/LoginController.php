...
class LoginController extends Controller
{
    ...
    public function loginForm(){
    	return view('superadmin.loginForm');
    }
    ...
    public function login(Request $request){
    	...
    	if (Auth::guard('superadmin')->attempt(['email' => $request->email, 'password' => $request->password, 'role_id'=> '0'])) {

           
            return redirect('superadmin/dashboard')->withSuccess('You are login successfully.');
        }
        ...
        if ($request->isMethod('post')) {
            ...
            if(!empty($admin)) {
                ...
                $mailData = [    
                                ...
                                'message' => url('superadmin/reset-password/'.$newSecurityToken)
                            ];
                $toEmail = $admin->email;
                Helper::sendMail($subject, $templateName, $mailData, $toEmail);
                return redirect('/superadmin/forgot-password')->withSuccess('Your reset password link sent on your email.');
            } else {
                return redirect('/superadmin/forgot-password')->withError('Email do not match with our records.');
            }
        }
    ...
    public function resetPassword(Request $request, $securityToken){
        if($request->isMethod('post')) {
            ...
            if($tokenData){
                ...
                return redirect('/superadmin/login')->withSuccess('Your password has been updated successfully.');
            }else{
                return redirect('/superadmin/login')->withError('Your link has been expired.');
            }
        }
        $checkToken = PasswordReset::where('token', $securityToken)->first();
        if(!empty($checkToken)){
            return view('superadmin.resetPassword', compact('securityToken'));
        }else{
            return redirect('/superadmin/login')->withError('Your link has been expired.');
        }
    }
    ...
    public function logout(){
        ...
        return redirect("superadmin/login")->withSuccess('You are logout successfully.');
    }
