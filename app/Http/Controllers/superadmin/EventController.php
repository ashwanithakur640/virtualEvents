...
class EventController extends Controller
{
            ...
            return DataTables::of($data)
                        ...
                        ->addColumn('action', function ($modal) {        
                            if($modal->start_date_time > Carbon::now()){
                                $avtiveEvents = '<a href="'.url('superadmin/events/edit-event/'.Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="Edit"><i class="fa fa-edit"></i></a>&nbsp';
                            } else{
                                $avtiveEvents = '';
                            }
                            $action = $avtiveEvents. '<a href="'.url("superadmin/events/show-event/".Helper::encrypt($modal->id)).'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>&nbsp<a class="deleteRecord" data-toggle="tooltip" title="Delete" style="color: #4e73df;"><i class="fa fa-trash"></i></a>
                                <form action="'.url('superadmin/events/delete-event/'.Helper::encrypt($modal->id)).'" method="POST" style="display: none;">
    ...
    public function eventParticipants(Request $request)
    {
        if ($request->ajax()) {
            if(!empty($request->start_date)  & empty($request->end_date) ){
                ...
                if(empty($request->start_date)  & empty($request->end_date)){
                    $data = User::where('users.role_id','2')
                                ->join('users as vendor','vendor.id','=','users.vendor_id')
                                ->select('users.*','vendor.first_name as vfname','vendor.middle_name as vmname','vendor.last_name as vlname')
                                ->get()->toArray();
                }
            }
            // print_r($data); die;
            return DataTables::of($data)
                        ...
                        ->addColumn('action', function ($modal) {        
                            ...
                            $action = '<a href="'.url("superadmin/events/show-event/".Helper::encrypt($modal['id'])).'" data-toggle="tooltip" title="View"><i class="fa fa-eye"></i></a>&nbsp<a class="deleteRecord" data-toggle="tooltip" title="Delete" style="color: #4e73df;"><i class="fa fa-trash"></i></a>
                                <form action="'.url('superadmin/events/delete-event/'.Helper::encrypt($modal['id'])).'" method="POST" style="display: none;">
                                    <input type="hidden" name="_token" value="'. csrf_token() .'">
                                    <input type="hidden" name="_method" value="DELETE">
                                </form>';
    ...
    public function todayEvents(Request $request){
        ...
        return view('superadmin.events.todays');
    }
    ...
    public function pastEvents(Request $request){
        ...
        return view('superadmin.events.past');
    }
    ...
    public function weeklyEvents(Request $request){
        ...
        return view('superadmin.events.weekly');
    }
    ...
    public function futureEvents(Request $request){
        ...
        return view('superadmin.events.list');
    }
    ...
    public function rescheduledEvents(Request $request){
        ...
        return view('superadmin.events.rescheduled');
    }
    ...
    public function holdEvents(Request $request){
        if ($request->ajax()) {
            ...
            return view('superadmin.events.hold');
    }
    ...
    public function create(){
    	...
    	return view('superadmin.events.create', compact('categories'));
    }
    ...
    public function store(Request $request){
        ...
        try{
        	...
        	return redirect('/superadmin/events/')->withSuccess('Event has been created successfully.');
    	} catch(\Exception $e) {
    ...
    public function edit($encryptId){
        ...
        $categories = Category::get();
        try{
            return view('superadmin.events.edit', compact('data', 'categories'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function update(Request $request, $encryptId){
        ...
        try{
            ...
            return redirect('superadmin/events/')->withSuccess('Event has been update successfully');
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
    ...
    public function eventShow($encryptId){
        ...
        $data = Event::findOrFail($id);
        try{
            return view('superadmin.events.view', compact('data'));
        } catch(\Exception $e) {
            return back()->withError($e->getMessage());
        }
    }
